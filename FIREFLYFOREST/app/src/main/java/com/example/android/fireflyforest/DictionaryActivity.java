        ...
        public void run() {
            ...
            try {
                ...
                if (temp[0] == null || temp[0].equals("")) {
                    System.out.println(searchedWord + "Server has no result!");
                    // 从网络上获取释义
                    w = dict.getWordFromInternet(searchedWord);
                    System.out.println("word:" + w.getWord());
                    System.out.println("Interpret:" + w.getInterpret());
                    if (w == null || w.getInterpret().equals("")) {
                        Looper.prepare();
                        Toast.makeText(DictionaryActivity.this, getString(R.string.no_result), Toast.LENGTH_SHORT).show();
                        Looper.loop();
                    } else {
                        words.clear();
