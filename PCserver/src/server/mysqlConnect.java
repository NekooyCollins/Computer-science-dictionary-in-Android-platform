...
public class mysqlConnect {
	...
	public static void createTables() {
		...
		try {
			...
			dictPstmt.executeUpdate(
					"create table if not exists dict(user_name varchar(50), word varchar(100), interpret varchar(50), likes int, hates int, score double);");
	...
	// 用户提交的释义存入dict表中
	// 新的释义插入时，根据来源不同设置不同的score初始值，金山为0.2，萤火虫官方为0.25
	...
	public static int updateUserLikeCounter(String wordInfo) throws SQLException {
		...
		// 当释义来源于官方且只有1点赞、0反对时，要特殊处理
		if(userName.equals("firefly forest")) {
			if((findTheLikeCounter(userName, word, trans) == 0) && (hateCounter == 0) && (Integer.parseInt(likeCounter) == 1))
				score = 0.28;
			else
